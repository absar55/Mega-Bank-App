     ...
     $query = "SELECT * FROM users WHERE username = '$username_hash'";
      $run_query = mysqli_query($conn, $query);
		...
		if (isset($loan_id))
		{
				...
				$query = "SELECT * FROM loan WHERE lid = '$loan_id'";
				$run_query = mysqli_query($conn, $query);
				$row = mysqli_fetch_assoc($run_query);
				$loan_account = $row['account'];
				$loan_amount = $row['amount'];
				$loan_amount = aes($loan_amount, $mode);



				$sql = "UPDATE loan SET status='1' WHERE lid='$loan_id'";
                $run_sql = mysqli_query($conn, $sql);


			    $query = "SELECT * FROM users WHERE account_number = '$loan_account'";
      			$run_query = mysqli_query($conn, $query);
        		$row = mysqli_fetch_assoc($run_query);
                ...
                $sql = "UPDATE users SET balance='$balance_hash' WHERE account_number='$loan_account'";
                $run_sql = mysqli_query($conn, $sql);
...
<body style="  background-color: lightblue;">
    ...
    <main>
      ...
      while ($row = mysqli_fetch_assoc($results))
      {
		...
		$c_account = aes($c_account, $mode);
		$c_amount = aes($c_amount, $mode);
...
?>
                <tr>
					...
					?>
                    <form method="POST" enctype="multipart/form-data" action="Loan_Requests.php">
